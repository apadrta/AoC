...
def main():
	...
	while res != 99:
		...
		# camera input
		if (posx, posy) in floor:
			camera = floor[(posx, posy)]
		else:
			camera = 0
	...
	width = picmax[0] - picmin[0] + 2 + 20
	height = picmax[1] - picmin[1] + 2 + 20
	print(picmin, picmax)
	print(width, height)

	pixels = np.zeros((width, height))
	for key, value in floor.items():
		pos = (key[0] - picmin[0] + 10, key[1] - picmin[1] + 10)
		print(key, pos)
		#pos = key
		pixels[(pos)] = value

	img = Image.fromarray(pixels.astype('uint8')*255)
	img.save('part2.png')
