...
def main():
	...
	#	print((cubex[i], cubey[i], cubez[i], cubew[i]))
		...
		for x in range(min(cubex) -1, max(cubex) + 2):
			for y in range(min(cubey) -1, max(cubey) + 2):
				for z in range(min(cubez) -1, max(cubez) + 2):
					for w in range(min(cubew) -1, max(cubew) + 2):
						# compute active neighbors
						active = 0
						for dx, dy, dz, dw in nb:
							#print("  ", [x + dx, y + dy, z + dz])
							if [x + dx, y + dy, z + dz, w + dw] in cubeall:
								active += 1
								#print("  ++ ", active)
						# evaluate
						#print("  ", active, [x, y, z] in cubeall)
						if [x, y, z, w] in cubeall and active in (2,3):
							#print(x, y, z)
							#print("  keep active")
							newcubes.append([x, y, z, w])
						if [x, y, z, w] not in cubeall and active == 3:
							#print(x, y, z)
							#print("  new active")
							newcubes.append([x, y, z, w])
		cubex = []
